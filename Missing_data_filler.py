...
Data=pd.read_csv("~/Documents/Machine Learning/Zillow/properties_2016.csv", low_memory=False)
Data=Data[600000:650000]
Data_filled_na=Data.copy()#.fillna(value=0)
del Data
to_del=['latitude','longitude','propertyzoningdesc', 'parcelid']
...
for to_predict in ls:
    Non_null_data=Data_filled_na[Data_filled_na[to_predict].notnull()].copy()
    if (len(Non_null_data)>=10) and (to_predict!='propertycountylandusecode') and (to_predict!='logerror'):
        ...
        int_msk=np.random.rand(len(Non_null_data))<0.05
        ...
        if score>=0.1:
            TP_mask=Data_filled_na[to_predict].notnull()
